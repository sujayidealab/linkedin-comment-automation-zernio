...
description: "Draft a LinkedIn comment on someone else's post from its URL, or reshare (repost) it to your feed with optional commentary. Use when the user pastes a post URL and asks to comment, engage, be first commenter, or repost with their thoughts. Produces 1-3 variants in the user's voice, picks a reaction, and publishes via Zernio on approval. Not for replying to existing comments (use linkedin-reply-handler)."
---
...
Then waits for user approval. On "post", calls Zernio to react + comment.
...
2. **Fetch the post body.** If `ZERNIO_API_KEY` is set, call `lib.ZernioClient.fetch_post(url)` for the post body and `fetch_post_comments(post_id=..., max_items=10)` for the top existing comments (so your draft doesn't duplicate an existing take). Both actors are no-cookies and cost roughly $0.001 + $0.005 per call on the Zernio free tier. If `ZERNIO_API_KEY` is not set, ask the user to paste the post text and (optionally) top comments.
...
7. **On approval.** Call `lib.publish(kind="comment", draft_text=<approved>, target_url=<post_url>, post_urn=<urn>, platform_id=<id>, reaction_type=<chosen>)`. The wrapper handles Zernio / manual / diy routing.
...
1. **Fetch the post** the same way (`lib.fetch_post(url)`), and check it is
   reshareable: the Zernio payload exposes `canShare` and the `shareUrn`
...
4. **On approval.** Call `lib.repost(post_url, commentary=<approved or None>)`.
   The wrapper resolves the correct `shareUrn` from Zernio (do not hand-convert an
   `activity` id, the share id can differ), refuses posts with resharing off, and
   routes Zernio / manual / diy. Manual tier returns copy-paste steps ("Repost
