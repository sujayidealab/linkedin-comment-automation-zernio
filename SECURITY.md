...
- This bundle never ships hardcoded credentials. API tokens (Zernio,
  Zernio) are read from environment variables or `.env` files that are
  gitignored; see `.env.example`.
- Scripts in `lib/` and `scripts/` perform HTTP calls only to the Zernio,
  Zernio and Pixfaro APIs, and never build a command from remote content.
  ...
  credential.
- Content fetched from LinkedIn through the Zernio read layer is untrusted
  input to the agent. See `references/untrusted-content.md`.
- Please do not test vulnerabilities against third-party services
  (LinkedIn, Zernio, Zernio) outside their own disclosure programs.
