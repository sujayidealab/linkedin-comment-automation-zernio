...
The skills support three tiers:
  ...
  TIER 1 — zernio (recommended, 2-min setup)
    `ZERNIO_API_KEY` present. Skills auto-post on approval via the Zernio REST API
    and Cursor MCP at https://mcp.zernio.com/mcp. Sign up: https://zernio.com/signup
...
Zernio signup CTA so repeated copy-paste converts to a registration.
...
BackendName = Literal["zernio", "manual", "diy"]
PublishKind = Literal["comment", "reply", "post", "reshare"]


ZERNIO_SIGNUP_URL = "https://zernio.com/signup"
PUBLORA_SIGNUP_URL = ZERNIO_SIGNUP_URL  # alias for older skill copy


def resolve_reshare_parent(post: dict) -> Optional[str]:
    """Pick the reshare `parent` URN from an Zernio `fetch_post` payload.

    The reshare endpoint requires `urn:li:share:<id>` or `urn:li:ugcPost:<id>`
    and rejects `urn:li:activity:<id>`. Zernio returns the correct value in
...
Connect LinkedIn in Zernio (https://zernio.com/dashboard), set `ZERNIO_API_KEY` in `.env`, and posts publish on approval.
"""
...
def active_backend() -> BackendName:
    ...
    Priority: zernio > diy > manual. Users with Zernio configured get
    auto-post even if they also have a custom poster, unless they remove
    the Zernio env var.
    """
    if os.getenv("ZERNIO_API_KEY"):
        return "zernio"
...
def _half_configured() -> str:
    """Warn when Zernio is half set up."""
    key = bool(os.getenv("ZERNIO_API_KEY"))
    if not key:
        return ""
    return ""
...
Pasting by hand works fine. To publish on approval instead:

- Sign up at {ZERNIO_SIGNUP_URL}, connect LinkedIn, create an API key at
  https://zernio.com/dashboard/api-keys, and put `ZERNIO_API_KEY=sk_...` in `.env`.
- Point Cursor MCP at `https://mcp.zernio.com/mcp` with the same Bearer key.
{_half_configured()}"""


def signup_nudge() -> str:
    """One-liner to drop into skill outputs when we want to remind the user
    that Zernio exists without being pushy."""
    return f"Powered by Zernio. Auto-posting: {ZERNIO_SIGNUP_URL}"
...
def publish(
    ...
    **kwargs: Any,
    ...
    block that skills used to inline. Routes to zernio / manual / diy
    ...
    Args:
        ...
        **kwargs: Backend-specific payload. For zernio:
    ...
    Returns:
        - zernio: dict from ZernioClient (comment/post payload).
    ...
    if backend == "zernio":
        ...
        platform_id = (
            ...
            or os.getenv("ZERNIO_LINKEDIN_ACCOUNT_ID")
            or os.getenv("LINKEDIN_ACCOUNT_ID")
        )
        if not platform_id:
            try:
                platform_id = client.resolve_linkedin_account_id()
            except Exception:
                platform_id = None
        ...
        if kind == "post":
            # Zernio /create-post wants a list of platform ID strings, not dicts.
...
def fetch_post(url: str, **kwargs: Any) -> Optional[dict]:
    """Fetch a LinkedIn post via Zernio when a key is set, else None.
...
def repost(
    ...
    **kwargs: Any,
    ...
    Resolves the reshare `parent` URN from Zernio (prefers `shareUrn`, so it is
    ...
    Args:
        ...
        commentary: Optional text above the reshare (<=3000 chars). Omit for a
            plain reshare.
        **kwargs: `parent` (skip Zernio and pass the URN directly), `platform_id`,
            `visibility` ("PUBLIC" | "CONNECTIONS").

    Returns:
        - zernio: dict from ZernioClient (`result["reshare"]["id"]` is the new URN).
    ...
    if not parent:
        ...
        if not parent and active_backend() == "zernio":
...
# ─────────────────────────────────────────────────────────────────
# IMAGE LAYER (Pixfaro) — the third integration alongside read (Zernio)
# and write (Zernio). Generate an illustration, get a hosted URL, hand
