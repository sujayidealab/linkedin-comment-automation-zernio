...
Note: the activity ID in the URL slug is NOT always the same as the canonical URN used by LinkedIn's backend (ugcPost vs activity vs share). For posting comments, use the post_urn returned here by default. If the direct URN 404s, fall back to resolving via `lib.ZernioClient.fetch_post_comments(post_id=...)` and read the canonical post URN from any existing comment's `post_input` field.
"""
