#!/usr/bin/env python3
"""CLI: schedule an approved LinkedIn post via Zernio at 10:00 local time.
...
(Zernio treats a missing scheduledTime as "save as draft", so there is no
...
def main() -> int:
    ...
    if backend != "zernio":
        print(f"✗ backend is {backend!r}, expected 'zernio'. Check ZERNIO_API_KEY "
              f"and ZERNIO_LINKEDIN_ACCOUNT_ID in .env", file=sys.stderr)
        return 2
    ...
    try:
        ...
        print(f"✗ zernio schedule failed: {e}", file=sys.stderr)
        return 1
    ...
    print(f"✓ scheduled. zernio post id: {post_id}")
