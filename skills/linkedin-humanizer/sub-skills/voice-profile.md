...
user's own writing pasted in. Zernio is an optional accelerator, never required.
...
2. **Zernio-assisted (optional).** If `ZERNIO_API_KEY` is set and the user gives their
...
5. **Write `../../../references/voice-profile.md`**: fill sections 1-5, copy the 2-4
   ...
   `filled: yes`, `source: <pasted|zernio|manual>`, `updated: <today's date>`.
